...
def process_frame(data_entry: dict) -> tuple[np.array, list[np.array]]:
    ...
    ball_tracking_boxes = process_boxes(data_entry["ball_tracking_boxes"])
...
def select_ball_object(tracked_objects: list[tuple[KalmanNDTrackerAdaptiveQ, int]]):
    """Select the ball based on most yellow color and less than 10 width and height"""
    max_yellow_score = -np.inf
    max_idx = None
...
def plot_frame(frame: np.array, tracked_object: tuple[KalmanNDTrackerAdaptiveQ, int]):
    ...
    # if len(tracked_object.previous_measurements) == 0:
    #     return frame
    # previous_measurement = tracked_object.previous_measurements[-1]
    # cv2.line(
    #     frame,
...
if __name__ == "__main__":
    ...
    offset = 1000
    for i in tqdm(range(len(tiafoe_video_frames[offset:3000]))):
        i += offset
        frame, measurements = process_frame(data[tiafoe_video_frames[i]])
